## File to test the g-iSE model on the UZH dataset with estimating lambda using RBPF ##
## Tests one setting of SG and GVar at a time; If using a clip for estimated lambda, change clipVal in line 38 ##
## Current runs for SG=10.0, GVar=100.0, and ClipVal=0.01 ##
...
## Getting data file ##
UZH = True
groundtruth_folder_path = Path("Data/UZH")
debugging_folder_base = "Debugging/conv_iSE/varyingLambda/UZH_Trajectories/RETEST_FalseGoal_HighSG_Clip"
...
for subfolder in groundtruth_folder_path.iterdir():
    if subfolder.is_dir(): #In a correct lambda subfolder
        track_difficulty = subfolder.name
        difficulty_specific_folder = os.path.join(debugging_folder_base, f"{track_difficulty}")
        os.makedirs(difficulty_specific_folder, exist_ok=True)
        print("Running for difficulty:", track_difficulty)
        ...
        all_runs_folder = os.path.join(difficulty_specific_folder, "all_runs")
        os.makedirs(all_runs_folder, exist_ok=True)

        #Keep overall RMSE and other results
        all_runs_rmse = []
        all_runs_rmse_normal = []
        ...
        all_runs_track_name = []
        all_runs_true_goal = []
        all_runs_predicted_goal = []
        all_runs_goal_at_convergence = []
        ...
        for track_file in track_files:
            ...
            groundtruth = make_groundtruth(track_file, UZH=True)
            #Sparsify
            groundtruth = groundtruth[::20]
            Tmax = groundtruth.shape[0]
            all_runs_time_total.append(Tmax)
            true_goal = groundtruth[-1]
            all_runs_true_goal.append(true_goal)
            ...
            #No longer a true lambda for these trajectories, so simply setting to a prior estimate
            ...
            for k in range(Tmax):
                m_pred_normal, v_pred_normal, F_aug_normal, _ = f.ise1_pred(t+dt*(k+1),mk_normal[-1],vk_normal[-1],s2,ls)
                normal_F_aug.append(F_aug_normal.copy())
                normal_predicted_means.append(m_pred_normal.copy())
                
                m_up_normal, v_up_normal, KGN, y_inN = f.update_ise1(noisy_data[k], m_pred_normal, v_pred_normal, sy)
                normal_updated_means.append(m_up_normal.copy())

                mk_normal.append(m_up_normal)
                vk_normal.append(v_up_normal)
                X_normal[k,:] = m_up_normal[0,:] + m_up_normal[-1,:] 
                S_normal[k] = v_up_normal[0,0] + v_up_normal[-1,-1]
            ...
            for k in range(Tmax):
                ...
                #See if goal region (5x5 box around) has been reached (in UZH case, considering the last point as the true goal)
                ...
                if inside_region and not reached_goal_yet and k > 0:
                    ...
                    all_runs_goal_at_convergence.append(overall_pred_goal)
                ...
                for key, value in particle_dict.items():
                    if value["lambda"] < clipVal:
                        value["lambda"] = clipVal
            ...
            overall_rmse_normal = get_model_rmse(X_normal, groundtruth)
            all_runs_rmse_normal.append(overall_rmse_normal)
            ...
            all_runs_predicted_goal.append(G_goal[-1])
            ...
            # Skip first time step's goal prediction as it's often incorrect
            save_tracking_plot(groundtruth, noisy_data, X_goal, G_goal[300:,:], "giSE-PF", "giSE_varyingLambda_debug.png", run_specific_folder, show_Target=True, true_goal=true_goal)
        ...
        mean_rmse_normal_iSE = np.mean(all_runs_rmse_normal)
        std_rmse_normal_iSE = np.std(all_runs_rmse_normal)
        ...
        with open(os.path.join(difficulty_specific_folder, "overall_results.txt"), 'w') as overall_results_file:
            overall_results_file.write("Lambda Prior Used: " + str(lambda_prior) + "\n")
            overall_results_file.write("Lambda_var: " + str(lambda_var) + "\n")
            overall_results_file.write("Clip value:" +str(clipVal)+"\n\n")
            ...
            overall_results_file.write("Mean g-iSE RMSE: " + str(mean_rmse) + "\n")
            overall_results_file.write("Std g-iSE RMSE: " + str(std_rmse) + "\n\n\n")

            overall_results_file.write("Mean iSE RMSE: " + str(mean_rmse_normal_iSE) + "\n")
            overall_results_file.write("Std iSE RMSE: " + str(std_rmse_normal_iSE) + "\n\n\n")
            ...
            overall_results_file.write("TRUE GOAL FOR EACH TRAJECTORY: \n")
            for i in range(len(all_runs_true_goal)):
                overall_results_file.write(f"\tRun " + str(all_runs_track_name[i]) + ": " + str(all_runs_true_goal[i]) + "\n")
            overall_results_file.write("\n")

            overall_results_file.write("PREDICTED GOAL FOR EACH TRAJECTORY: \n")
            for i in range(len(all_runs_predicted_goal)):
                overall_results_file.write(f"\tRun " + str(all_runs_track_name[i]) + ": " + str(all_runs_predicted_goal[i]) + "\n")
            overall_results_file.write("\n")
            ...
            overall_results_file.write("ACTUAL TIME TAKEN FOR EACH TRAJECTORY: \n")
            for i in range(len(all_runs_time_total)):
                overall_results_file.write(f"\tRun " + str(all_runs_track_name[i]) + ": " + str(all_runs_time_total[i]) + "\n")
            overall_results_file.write("\n")

            overall_results_file.write("ACTUAL GOAL AT TIME WE REACHED GOAL: \n")
            for i in range(len(all_runs_goal_at_convergence)):
                overall_results_file.write(f"\tRun " + str(all_runs_track_name[i]) + ": " + str(all_runs_goal_at_convergence[i]) + "\n")
            overall_results_file.write("\n")
