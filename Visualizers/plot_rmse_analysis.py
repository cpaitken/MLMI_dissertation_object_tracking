...
def parse_convergence_statistics(filename):
    ...
    for section in sections[1:]:  # Skip the first empty section
        ...
        if sigma_g_match:
            ...
            rmse_match = re.search(r'Mean RMSE: ([\d.]+) ± ([\d.]+)', section)
            if rmse_match:
...
def plot_rmse_vs_sigma_g(rmse_data, save_path=None):
    ...
    # Customize the plot
    plt.xlabel('σ_g (Goal Process Noise)', fontsize=12)
    plt.ylabel('Mean RMSE', fontsize=12)
    plt.title('Effect of σ_g and G_var on Tracking RMSE', fontsize=14, fontweight='bold')
    plt.legend(title='G_var Values', fontsize=10)
...
def main():
    ...
    # Parse the data
    print("Parsing convergence statistics...")
